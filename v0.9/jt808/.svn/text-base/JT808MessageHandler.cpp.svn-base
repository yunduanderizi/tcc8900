...
JT808MessageHandler::JT808MessageHandler()
{
    ...
    m_waitingQueue_0900.set_capacity(JT808PacketFifo::capacity_type(JT808_MAX_WAITING_QUEUE));
...
void JT808MessageHandler::ClearQueue()
{
    ...
    for(JT808PacketFifo::iterator it = m_waitingQueue_0900.begin(); it != m_waitingQueue_0900.end(); it ++)
    {
        StruJT808Packet *packet = *it;
        DestroyPacket(packet);
    }
    m_waitingQueue_0900.clear();
...
void JT808MessageHandler::RefreshConfig()
{
    ...
    printf("fei-----------------------RefreshConfig\n");
    // int ii = read_int();
    int ii =0;
	printf("fei-------------------------after  read_int\n");
    //loop_net();
...
void JT808MessageHandler::PopQueue(int id, int seq)
{
    ...
    if(id == JT808_PACKET_LOC_MULTI_REPORT)
   {
	    ...
	    packet = *(m_waitingQueue_location.begin());
   ...
   {
   	  printf("fei --------------------------   befoe   m_waitingQueue_0900         %d  \n",m_waitingQueue_0900.size());
	  
	   if (m_waitingQueue_0900.size()>=1)
	  {
			LogUtility::Log(LOG_LEVEL_DEBUG, "JT808MessageHandler::PopQueue m_waitingQueue_0900 reply_flag =%d .",reply_flag);
			packet = *(m_waitingQueue_0900.begin());
			printf("PopQueue::m_waitingQueue_0900.size()==%d \n",m_waitingQueue_0900.size());
		       LogUtility::Log(LOG_LEVEL_DEBUG, "JT808MessageHandler:: pop m_waitingQueue_0900 retry %d  resp(%d, %04x).", packet->retry,packet->seq, packet->id);
		   	 if((packet->retry == 0) && (packet->id == id) && (packet->seq == seq)&&reply_flag==1)
		   	 {
			       LogUtility::Log(LOG_LEVEL_DEBUG, "JT808MessageHandler::pop m_waitingQueue_0900 correct resp.");
				printf("fei -------------------------------------------1121212\n");
				m_waitingQueue_0900.pop_front();
				printf("fei -------------------------------------------1132323\n");
				reply_flag=0;
				printf("fei -------------------------------------------1143434\n");
				StartTimer(m_queueTimer);
	    ...
	    if(m_waitingQueue.size() <= 0)
	    {
            ...
            printf("fei -------------------------- 1111111111111111111111111111\n");
...
void JT808MessageHandler::HandleTimer(string &name, void *data)
{
	...
	{
	       printf("fei--------------------JT808_DEAD_ZONE_TIMER\n");
...
#endif
		case JT808_PACKET_UP_TRANSPARENT_TRANSMISSION:
			if(m_waitingQueue_0900.size() >= JT808_MAX_WAITING_QUEUE_LOCATION)
			{
				LogUtility::Log(LOG_LEVEL_DEBUG, "jt808messagehandler::pushqueue full");
				pthread_mutex_unlock(&m_queueMutex);
				DestroyPacket(packet);
			...
			printf("fei -PushQueue- packet->id  = %x  , packet->seq  =%d,m_waitingQueue_0900=%d\n",packet->id , packet->seq,m_waitingQueue_0900.size());
			m_waitingQueue_0900.push_back(packet);
			pthread_mutex_unlock(&m_queueMutex);
			if(m_waitingQueue_0900.size() == 1)
			{
				LogUtility::Log(LOG_LEVEL_DEBUG, "jt808messagehandler::pushqueue empty");
...
void JT808MessageHandler::SendQueue()
{
	...
	if(m_authorized==true&&s_multipacket==false)
	{
		...
		/****************************************************************************/
		if(m_waitingQueue_0900.size() <= 0)
		{
			LogUtility::Log(LOG_LEVEL_DEBUG, "JT808MessageHandler::SendQueue0 empty");
		}
		else
		{
			printf(" ----------------SendQueue send m_waitingQueue_reply  reply_flag==%d , m_waitingQueue_0900.size() =%d\n",reply_flag,m_waitingQueue_0900.size());
			StruJT808Packet *packet = *(m_waitingQueue_0900.begin());
			if (reply_flag==0)
			{
				SendPacket(packet);
				reply_flag=1;
...
{
	m_send_len = 0;
	printf(" SendPacket   1   m_waitingQueue_0900.size()   %d \n ",m_waitingQueue_0900.size());
	vector<JT808Packet*> &packetarray = packet->packetarray;
	if(packetarray.size())
	{
		if(packet->retry == 0)
	...
	else
	{
		...
		if(packet->reply)
		{
			...
			//if((packet->id == JT808_PACKET_LOC_MULTI_REPORT)||(packet->id ==JT808_PACKET_UP_TRANSPARENT_TRANSMISSION)) 
			if( packet->id == JT808_PACKET_LOC_MULTI_REPORT)
			{
				m_waitingQueue_location.pop_front();
                      ...
                      {
				if (packet->id !=JT808_PACKET_UP_TRANSPARENT_TRANSMISSION )
					m_waitingQueue_reply.pop_front();
                      }
			printf(" SendPacket   2  m_waitingQueue_0900.size()   %d \n ",m_waitingQueue_0900.size());
			if(m_waitingQueue_reply.size() > 0)
			{
				if (packet->id != JT808_PACKET_UP_TRANSPARENT_TRANSMISSION)
					DestroyPacket(packet);
			...
			if(m_waitingQueue_0900.size() > 0 && ((*m_waitingQueue_0900.begin())->id!=packet->id))
			{
				printf(" SendPacket  3   m_waitingQueue_0900.size()   %d \n ",m_waitingQueue_0900.size());
				LogUtility::Log(LOG_LEVEL_DEBUG, "JT808MessageHandler::m_waitingQueue_0900 not empty");
				StartTimer(m_queueTimer);
				return ;
			}
			if (packet->id != JT808_PACKET_UP_TRANSPARENT_TRANSMISSION)
...
int JT808MessageHandler::SendContent(JT808Packet &content)
{
	...
	LogUtility::Log(LOG_LEVEL_ERROR, "JT808MessageHandler::SendContent ret=%d\n",ret);
    if( ret != m_send_len )
    {
...
void JT808MessageHandler::PacketHeadGZ( JT808Packet &packet, unsigned short id, ENUM_JT808_CRYTO crypto)
{
    printf("fei ---------------- PacketHeadGZ\n");	
    unsigned short len;
    ...
    LogUtility::Log(LOG_LEVEL_DEBUG, "fei ----------------------------------JT808MessageHandler::PacketHead Count len %d", len);
...
void JT808MessageHandler::PacketCheckSumFlagGZ(JT808Packet *packet)
{
    unsigned char checksum = 0;
    int i;
   
	printf("fei ------------------------PacketCheckSumFlag\n");
    for(JT808Packet::iterator it = packet->begin(); it != packet->end(); it ++)
    {
        checksum ^= (*it);
    }
