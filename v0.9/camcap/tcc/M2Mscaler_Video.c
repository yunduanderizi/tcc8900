...
void M2M_ScalerForVideo_Enable(int yuv_mode)
{
	...
	/*映射设备文件从0xF0220000开始的1024*4大小到M2MBaseAddress*/
	...
	if(!M2MBaseAddress)
	{
		...
		FreeVirtual(&vM2MScaler, (void *)&M2MBaseAddress ,PHYSICAL_REG_SIZE);
		return;
	}
	#ifdef TCC_89XX_INCLUDE
/*
...
{	
...
#if 0
	#ifdef TCC_89XX_INCLUDE //moidfy recovery fb, mplayer can not play video.
	BITSET(pClockReg->SWRESET, Hw6);
	...
	if(pClockReg){
		FreeVirtual(&vM2MScaler, (void *)pClockReg		,PHYSICAL_REG_SIZE);		
	}
	...
	fprintf(stdout, "M2M_ScalerForVideo_Disable-------finish\n");
}
...
//	write_val |= 0x08; // interleave mode
//	write_val |= 0x03; // yuv420 mode
	write_val |= 0x02; //yuv422 separate
...
//#endif
	...
	if(hdmi_flag)
		...
		BITCSET(HwREG(M2MBaseAddress+0x34), 0x0000000f, write_val);//HwDSTCFG_422SEQ0);
...
void M2M_ScalerForVideo_With_Address_noviqe( unsigned int s_hsize, unsigned int s_vsize, unsigned int m_hsize, unsigned int m_vsize, unsigned int s_y, unsigned int s_u, unsigned int s_v, unsigned int d_y , unsigned int v_offset, unsigned int h_offset, int hdmi_flag) 
{
	...
	BITCSET(HwREG(M2MBaseAddress+0x34), 0x000000ff,write_val);// HwDSTCFG_422SEQ0);
...
void M2M_ScalerForVideo_With_Address_noviqe_bak( unsigned int s_hsize, unsigned int s_vsize, unsigned int m_hsize, unsigned int m_vsize, unsigned int s_y, unsigned int s_u, unsigned int s_v, unsigned int d_y , unsigned int v_offset, unsigned int h_offset, int hdmi_flag) 
{
	unsigned int		write_val;
	volatile unsigned int  IsM2MScalerEnd ;
	unsigned long 		IFlag;
...
printf("---------%s    %d\n",__func__,__LINE__);
	...
	pClockReg1			= (DDICONFIG *) SetVirtual(&vM2MScaler1, (unsigned int)&HwDDI_CONFIG_BASE, PHYSICAL_REG_SIZE);
	...
	M2MBaseAddress1	= (unsigned int)SetVirtual(&vM2MScaler1, (unsigned int)&HwMSC_BASE, PHYSICAL_REG_SIZE);	
	if(!M2MBaseAddress1)
	{
		printf("M2MBaseAddress Alloc Error!\n");		
		FreeVirtual(&vM2MScaler1, (void *)M2MBaseAddress1 ,PHYSICAL_REG_SIZE);
		return;
	}

	M2MSrcAddress = (unsigned int)SetVirtual(&vM2MScaler1,M2MSrcPhyAddr,SCALER_LIMIT);
	printf("M2MSrcAddress : %08x \n",M2MSrcAddress);
	if(!M2MSrcAddress)
	{
		printf(">>>>>>>>>>M2MBaseAddress Alloc Error!\n");		
		FreeVirtual(&vM2MScaler1, (void *)M2MSrcAddress ,SCALER_LIMIT);
		return;
	}
	...
	M2MDstAddress = (unsigned int)SetVirtual(&vM2MScaler1,FB2_PAddr,SCALER_LIMIT);
	
	if(!M2MDstAddress)
	{
		printf("M2MBaseAddress Alloc Error!\n");		
		FreeVirtual(&vM2MScaler1, (void *)M2MDstAddress ,SCALER_LIMIT);
		return;
	}
...
{	
	...
	if(pClockReg1){
		FreeVirtual(&vM2MScaler1, (void *)pClockReg1		,PHYSICAL_REG_SIZE);		
	}
