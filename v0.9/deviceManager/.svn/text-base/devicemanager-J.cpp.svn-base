...
void process_gps_position_info(int itime)
{
    ...
    //if((vdr_alarm_flag & ALARM_STATUS_IN_OUT_AREA)||(vdr_alarm_flag & ALARM_STATUS_IN_OUT_LINE))
...
void process_collision_side_turn_alarm()
{
    ...
    ioctl(cst_fd, 0x02, &t_flag);   // turnover flag
...
#if 1            
            printf("position=%d, fx=%d, fy=%d, itime=%d,isystime=%d,iOrientation=%d,iVel=%d,ilat=%d,ilon=%d,ialt=%d  %d mode=%d\n",
                gps_buf.position,gps_buf.fX,gps_buf.fY,gps_buf.iTime,gps_buf.iSysTime,gps_buf.iOrientation,gps_buf.iVelocity,
                gps_buf.ilatitude, gps_buf.ilongitude,gps_buf.ialtitude,hx_info_num,gps_buf.mode);
#endif
            #ifdef BLIND_ZONE
