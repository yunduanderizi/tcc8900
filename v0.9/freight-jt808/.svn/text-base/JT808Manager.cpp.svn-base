...
int mangqu=0;                            // gnss send flag 
int  wire_state =0;
volatile int  mangqu_flag = 0;        //fence flag 
	...
	while(1)
	{
		...
		{    
			...
			else if((ConnectionManager::Instance()->b_zone == true) &&mangqu_flag == 1)  //Communication close
			{
			       ...
			       printf("****************************  Befor in  elseif    fence_inout_flag = %d\n", fence_inout_flag);
				if( fence_inout_flag == 1 )
				{
					printf("!!!!!!!!!!!!!!**************** This is in gps thread **************************\n");
...
void CNaviElectronicFenceInorOutNotifyHandler(DBusNotifyC *pNotify)
{
	...
	printf("!!!!!!!********************This is  in CNaviElectronicFenceInorOutNotifyHandler ************************** \n");
	int gnss_tmp ;
	...
	if(Notify->flag == 1) // 进区域
	{
		...
		else    //gnss  不采集 
		{
			...
			printf("CNaviElectronicFenceInorOutNotifyHandler  fence_inout_flag +%d\n", fence_inout_flag);
