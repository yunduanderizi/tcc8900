...
extern text SendText;
extern question SendQuestion;
extern callback SendCallBack;
extern notify SendContactsUpdate;
extern notify SendSettingUpdate;
extern regist SendLogoff;
extern regist SendRegisterN;
extern regist SendQuickRegister;
extern regist SendAuthentication;
...
void JT808MessageHelper::Init(JT808MessageHandler *handler)
{
    ...
    JT808Paras.insert(map<int, StruJT808Para>::value_type(0x0081, StruJT808Para((int)VDR_DEPLOY_PARA_PROVINCE_ID,        ENUM_SQLite_COLUMN_INTEGER,     2)));
    JT808Paras.insert(map<int, StruJT808Para>::value_type(0x0082, StruJT808Para((int)VDR_DEPLOY_PARA_CITY_ID,            ENUM_SQLite_COLUMN_INTEGER,     2)));
    JT808Paras.insert(map<int, StruJT808Para>::value_type(0x0083, StruJT808Para((int)VDR_DEPLOY_PARA_PLATE_ID,           ENUM_SQLite_COLUMN_STRING,      0)));
    JT808Paras.insert(map<int, StruJT808Para>::value_type(0x0084, StruJT808Para((int)VDR_DEPLOY_PARA_PLATE_COLOR,        ENUM_SQLite_COLUMN_INTEGER,     1)));
...
void JT808MessageHelper::SendAttribute(unsigned short seq)
{
    ...
    vector<unsigned char> manuID, modelNO, modelID,simNO;
    string hardvers,firmvers;
    ...
    m_handler->PushSequence(packet, modelNO, 20);
    m_handler->PushSequence(packet, modelID, 7);
    m_handler->PushSequence(packet, simNO, 10);
...
void JT808MessageHelper::SendLocationQueryResp(unsigned short seq)
{
    ...
    {
        LogUtility::Log(LOG_LEVEL_ERROR, "JT808MessageHelper::SendLocationQueryResp failed to RefreshBottom");
        delete table;
        return;
    }
...
void JT808MessageHelper::SendLocationMulti(struct LocStatusData *locData,  int num, int lock_door)
{
	...
	m_handler->PushByteFront(packet, 1);
	m_handler->PushWordFront(packet, num);
	m_handler->SendBody(thepacket, JT808_PACKET_LOC_MULTI_REPORT, JT808_CRYTO_NONE);
}
...
//		    JT808RegisterDbusNotifyC notify;
		    if((result == 0x00) || (result == 0x05))
		    {
			    ...
			    delete table;
//			    notify.result=0x0A;
			    quick_register_result=0x0A;
		    }
		    else
			   quick_register_result=result;
		   SendQuickRegister(quick_register_result);
...
//            SendJT808LocReportNotify(result);
	      JT808MessageHelper::SendJT808LocReportNotify(result);
//            devicemanager相关
            break;
...
//    notify.flag = 0;
	...
	else if(para.type == ENUM_SQLite_COLUMN_STRING)
	{
		...
		if(id<0x80||id>=0x90)
			table->SetStringValue(para.colid, value);
		else 
			deploytable->SetStringValue(para.colid, value);
	}
...
//    调用外部接口函数
      SendSettingUpdate();
      JT808MessageHelper::SendJT808SettingUpdateNotifyC();
    if(flag_gnss ==1 )
    {
...
//	    SendJT808GnssUpdateNotify(gnss_mode,gnss_baud,gnss_out,gnss_fre);
	    JT808MessageHelper::SendJT808GnssUpdateNotify(gnss_mode,gnss_baud,gnss_out,gnss_fre);
//	    devicemanager相关
    }
    #ifdef HBTEST
    JT808MessageHelper::SendJT808ParkingNotify(flag);
//    g_pDbusServer->sendNotify(&notify);
//    devicemanager 相关
    #endif
    return;
...
//	    ret = g_pDbusClient->sendService(&srequest,reply,5*1000);
	    JT808MessageHelper::SendJT808ShutDownNotify(); 
//	    devicemanager相关
#if 0
	    if(ret < 0)
	    {
...
//	    ret = g_pDbusClient->sendService(&rrequest,reply,5*1000);
	    JT808MessageHelper::SendJT808RebootNotify();
//	    devicemanager相关
#if 0
	    if(ret < 0)
	    {
...
void JT808MessageHelper::HandleTempLocControl(JT808Packet &packet, int body, int len, unsigned short seq)
{
    ...
    body += 2;
    int duration = JT808_GET_LONG(packet, body);
...
//    SendJT808TempLocReportNotify(interval,duration);
    JT808MessageHelper::SendJT808TempLocReportNotify(interval,duration);
//    devicemanager相关
...
//    devicemanager相关
    JT808MessageHelper::SendJT808LocReportNotify(confirm); 
    return;
}
...
//    调用外部接口函数
    SendText(flag&0x01, flag&0x04, flag&0x08, flag&0x10, flag&0x20,utf_value);
...
void JT808MessageHelper::HandleQuestion(JT808Packet &packet, int body, int len, unsigned short seq)
{
  ...
  //  调用外部接口函数
    SendQuestion(seq, flag & 0x01, flag & 0x10, flag & 0x08, flag & 0x10, utf_question, answers);
...
//			SendJT808FenceUpdateNotify(0,0,0,-1);	
  			JT808MessageHelper::SendJT808FenceUpdateNotify(0,0,0,-1);
//			devicemanager相关
			flag=1;
		}
		...
		if(attr==ENUM_JT808_FENCE_MODIFY)
			JT808MessageHelper::SendJT808FenceUpdateNotify(0,0,id,0);
//			SendJT808FenceUpdateNotify(0,0,id,0);
//			devicemanager相关
		else if((attr==ENUM_JT808_FENCE_APPEND)||(attr==ENUM_JT808_FENCE_UPDATE))
			JT808MessageHelper::SendJT808FenceUpdateNotify(1,0,id,0);
...
void JT808MessageHelper::HandleSetRectangle(JT808Packet &packet, int body, int len, unsigned short seq)
{
    ...
    while(accu_len < len)
    {
		...
		if(flag==0){
			...
			JT808MessageHelper::SendJT808FenceUpdateNotify(0,1,0,-1);
...
//			devicemanager相关
		...
		if(attr==ENUM_JT808_FENCE_MODIFY)
			JT808MessageHelper::SendJT808FenceUpdateNotify(0,1,id,0);
//			SendJT808FenceUpdateNotify(0,1,id,0);
//			devicemanager相关
		else if((attr==ENUM_JT808_FENCE_APPEND)||(attr==ENUM_JT808_FENCE_UPDATE))
			JT808MessageHelper::SendJT808FenceUpdateNotify(1,1,id,0);
...
// 			devicemanager相关  
    ...
    JT808MessageHelper::SendJT808RectangleFenceUpdateNotify();    
...
void JT808MessageHelper::HandleSetPolygon(JT808Packet &packet, int body, int len, unsigned short seq)
{
	...
	if(table->Commit() < 0)
	{
        ...
        JT808MessageHelper::SendJT808FenceUpdateNotify(1,2,id,0);	
...
void JT808MessageHelper::HandleDeletePolygon(JT808Packet &packet, int body, int len, unsigned short seq)
{
	...
	JT808MessageHelper::SendJT808FenceUpdateNotify(0,2,0,-1);
...
void JT808MessageHelper::HandleSetLine(JT808Packet &packet, int body, int len, unsigned short seq)
{
	...
	if(table->Commit() < 0)
	{
        ...
        JT808MessageHelper::SendJT808FenceUpdateNotify(1,3,id,0);	
...
void JT808MessageHelper::HandleDeleteLine(JT808Packet &packet, int body, int len, unsigned short seq)
{
	...
	JT808MessageHelper::SendJT808FenceUpdateNotify(0,3,0,-1);
...
//					printf("%d::name is %s, lon is %f ,lat is %f \n",__LINE__,arrayPOI[i].strName.c_str(), arrayPOI[i].dlon, arrayPOI[i].dlan);
		...
		case JT808_TT_IC: 
			...
			notify.errnum=0x00;
			memcpy(buff_data,&notify,sizeof(notify));
			m_handler->to_device->Write(buff_data,sizeof(notify));
//			g_pDbusServer->sendNotify(&notify);
			break;
		default:
...
int JT808MessageHelper::SendJT808LocReportNotify(int result)
{
	unsigned char buffer[128];
	JT808LocReportNotify notify;
	notify.command=1;
	notify.alarm=result;
	memcpy(buffer,&notify,sizeof(notify));
	m_handler->to_device->Write(buffer,sizeof(notify));
	return 1;
}
int JT808MessageHelper::SendJT808TempLocReportNotify(int interval, int validity)
{
	unsigned char buffer[128];
...
void JT808MessageHelper::SendJT808SettingUpdateNotifyC()
{
	unsigned char buffer[128];
...
void JT808MessageHelper::SendJT808RebootNotify()
{
	unsigned char buffer[128];
	RebootMachineReqC notify;
	notify.command=4;
	memcpy(buffer,&notify,sizeof(notify));
	m_handler->to_device->Write(buffer,sizeof(notify));
}
void JT808MessageHelper::SendJT808ShutDownNotify()
{
	unsigned char buffer[128];
	ShutdownMachineReqC notify;
	notify.command=3;
	memcpy(buffer,&notify,sizeof(notify));
	m_handler->to_device->Write(buffer,sizeof(notify));
}
int JT808MessageHelper::SendJT808GnssUpdateNotify(int mode,int baud,int out,int fre)
{
	unsigned char buffer[128];
...
void JT808MessageHelper::SendJT808FenceUpdateNotify(int flag,int type,int id,int state)
{
	unsigned char buffer[128];
...
void JT808MessageHelper::SendJT808RectangleFenceUpdateNotify()
{
	unsigned char buffer[128];
...
void JT808MessageHelper::SendJT808ParkingNotify(int flag)
{
	unsigned char buffer[128];
	JT808ParkingDbusNotifyC notify;
	notify.command = 8;
